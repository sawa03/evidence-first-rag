...
- 基于 Python 公开文档的 100 题中文自建挑战集、主题划分、分题型报告与逐题结果（待独立人工复核）。
...
默认 Atlas 演示共有 8 个资料块、14 个问题（10 个有答案，4 个无答案）。这是与实现一起编写的合成冒烟集，未进行独立测试集划分，不能代表公开基准或实际业务准确率。
...
新增 [Python 文档中文挑战集](data/python_docs/README.md)：16 块原创事实笔记，100 道 AI 辅助编写的问题，含参考答案与官方出处，尚未经过独立人工复核。开发/测试各 50 题，按主题及相关证据分开；同主题题目有相关性，不能视为 100 个独立知识点。默认网页仍使用 Atlas，评测不会重启服务或替换正在使用的语料。

固定默认 BM25、k=3、门槛 0.15 的首次离线基线：

| 指标 | 开发集 | 测试集 |
| --- | --- | --- |
| 有答案问题 Recall@3 | 97.06% | 100% |
| 有答案问题 MRR@3 | 95.59% | 97.06% |
| 无答案正确拒答 | 12/16 | 3/16 |
| 无答案错误放行 | 4/16 | 13/16 |
| 有答案问题接受 | 31/34 | 32/34 |

测试集的检索命中高，但无答案错误放行严重：词项重合不能证明证据充分。以上是摘录模式结果，**没有调用 GPT，也不是回答正确率**。默认参数在运行测试集前已固定；后续优化仅使用开发集，需要新题验证泛化。完整命令见数据集说明，逐题输出见 [开发报告](reports/python-dev.json)、[测试报告](reports/python-test.json)、[基线分析](docs/BASELINE.md)。
